...
const svg =
    <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns="http://www.w3.org/1999/xlink" x="0px" y="0px"
        viewBox="0 0 54 54" xml="preserve">
        <g fill="white">
...
export default class ReviewList extends Component {
    ...
    load = () => {
        var reviews = reviewService.query()
        this.setState({ reviews })
    }

    onRemove = (id) => {
        var newList = reviewService.remove(id)
    ...
    render() {
        ...
        return !reviews ? 'loading' : (
            <section>
                <section className="widget flex">
                    <div className="icon-container">
                        {svg}
                    </div>
                    <p>
                        התהליך עדיין פתוח לשינוי. הצוות יוכל לראות את המסקנות הסופיות רק לאחר שיאושרו על ידי ההנהלה.
                    </p>
                </section>
                <section className="review-list">
                    {
                        reviews.map((review, idx) =>
                            <Link to={'/:' + review.id} exact='true' className="review-preview" key={review.id}>
                                <ReviewCard review={review} idx={idx + 1} onRemove={this.onRemove} onSave={this.onSave} />
                            </Link>
                        )
                    }
                </section>
